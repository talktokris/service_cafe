...
export default function EditStockItemSettingsComponent({
    stockItem,
    onClose,
    onSuccess,
    officeProfiles = [],
    ...
    // Initialize form data when stockItem prop changes
    useEffect(() => {
        if (stockItem) {
            setFormData({
                headOfficeId: stockItem.headOfficeId || "",
                branchId: stockItem.branchId || "",
                itemName: stockItem.itemName || "",
                itemType: stockItem.itemType || "food",
                activeStatus: stockItem.activeStatus || 0,
            });
        }
    }, [stockItem]);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            router.put(`/stock-item-settings/${stockItem.id}`, formData, {
                onSuccess: (page) => {
                    // Show success message
                    if (page.props.flash?.success) {
                        console.log(page.props.flash.success);
                    }
                    // Call the success callback with the updated stock item data
                    if (page.props.stockItemSetting) {
                        onSuccess(page.props.stockItemSetting);
                    } else {
                        // If no stock item data in response, create updated stock item for the callback
            ...
            console.error("Error updating stock item setting:", error);
            setIsSubmitting(false);
        }
    };
                ...
                <div className="p-6 border-b border-gray-200">
                    <div className="flex items-center justify-between">
                        <h2 className="text-xl font-semibold text-gray-900">
                            Edit Stock Item Setting: {stockItem.itemName}
                        </h2>
                        <button
                            onClick={onClose}
